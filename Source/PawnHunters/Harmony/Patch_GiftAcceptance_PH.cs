...
// Allows the player to send drop pod gifts to PH settlements despite permanentEnemy=true.
...
public static class Patch_TransportersArrivalAction_GiveGift_CanGiveGiftTo_PH
{
        ...
        ref FloatMenuAcceptanceReport __result)
    {
        ...
        if (settlement?.Faction?.def != PawnHuntersDefOf.PH_PawnHunters) return;
...
// Allows the player to offer caravan gifts to PH settlements.
// Skips both the permanentEnemy check and the CanTradeNow check (PH has no TraderKindDef).
[HarmonyPatch(typeof(CaravanArrivalAction_OfferGifts), nameof(CaravanArrivalAction_OfferGifts.CanOfferGiftsTo))]
public static class Patch_CaravanArrivalAction_OfferGifts_CanOfferGiftsTo_PH
{
