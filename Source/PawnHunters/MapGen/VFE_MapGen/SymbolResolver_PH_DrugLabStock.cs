...
/// Drug storage room — removes the PH_DrugMedicine marker, clears shelves, fills ~3/4 with
///   PH_DrugMedicine and ~1/4 with PH_BP (all stacks 60–100% full). Sets wall coolers to -9°C.
/// </summary>
public class SymbolResolver_PH_DrugLabStock : SymbolResolver
{
    ...
    {
        // Each PH_DrugLab structure places one PH_DrugMedicine item as a room marker.
        // Collect one representative marker per distinct room and process each independently.
        var markersByRoom = map.listerThings.ThingsOfDef(PawnHuntersDefOf.PH_DrugMedicine)
        ...
        foreach (var (marker, room) in markersByRoom)
        {
            ...
            SpawnItemsOnCells(cells.ToList(), PawnHuntersDefOf.PH_DrugMedicine, map);
    ...
    {
        var markersByRoom = map.listerThings.ThingsOfDef(PawnHuntersDefOf.PH_BP)
            .Where(t => settlementRect.Contains(t.Position) && t is not Building)
            .GroupBy(t => t.GetRoom())
            .Where(g => g.Key != null)
            .Select(g => (marker: g.First(), room: g.Key!))
        ...
        foreach (var (marker, room) in markersByRoom)
        {
            ...
            SpawnItemsOnCells(cells.ToList(), PawnHuntersDefOf.PH_BP, map, 1, 6);
        }
    }
