...
public class SymbolResolver_PH_Prisoners : SymbolResolver
{
    ...
    public override void Resolve(ResolveParams rp)
    {
        ...
        var extractorPositions = map.listerThings.GetThingsOfType<Building>()
            .Where(b => b.def == PawnHuntersDefOf.PH_BP_Extractor && rp.rect.Contains(b.Position))
            .Select(b => b.Position)
            .ToList();
    ...
    private static Pawn GeneratePrisoner(Map map, bool geneMatchRequired, Faction captorFaction)
    {
        ...
        // Always use one if this is the last PH settlement; otherwise 5% chance.
            ...
            if (isLastSettlement || Rand.Chance(0.05f))
            {
                ...
                if (kidnappedList?.Count > 0)
                {
                    ...
                    if (kidnapped != null)
                    {
                        ...
                        {
                            var settings = PawnHuntersMod.Settings;
    ...
    private static void TryApplyTargetGenes(Pawn pawn, PHModSettings settings)
    {
        ...
        if (settings.targetGenes?.Count > 0)
        {
            ...
            if (genes.Count > 0)
            {
                GeneUtility.UpdateXenogermReplication(pawn);
                pawn.genes.SetXenotype(XenotypeDefOf.Baseliner);
                pawn.genes.xenotypeName = "PH_SelectedGenes".Translate();
