...
///      raw human meat, and optional drug medicine / BP stash.
///   3. Guards — spawns 0–2 PH fighters near the bedroom beds.
///   4. Mines — scatters IED / spike traps in a ring around the structure.
/// </summary>
public class SymbolResolver_PH_HuntingHutPrison : SymbolResolver
{
    ...
    private static void FillGuardShelf(Map map, CellRect rect, Faction captorFaction)
    {
        ...
        // 5% chance: single BP on shelf
        if (Rand.Chance(0.05f))
        {
            Thing bp = ThingMaker.MakeThing(PawnHuntersDefOf.PH_BP);
            GenSpawn.Spawn(bp, cells.RandomElement(), map);
        }
    }
    ...
    private static void SpawnGuards(Map map, CellRect rect, Faction captorFaction)
    {
        ...
        Faction? hunterFaction = Find.FactionManager.FirstFactionOfDef(PawnHuntersDefOf.PH_PawnHunters);
