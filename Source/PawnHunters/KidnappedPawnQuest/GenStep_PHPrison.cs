...
public class GenStep_PHPrison : GenStep
{
    private const string PrisonSitePartDefName = "PH_KidnappedPawnPrison";
    ...
    public override void Generate(Map map, GenStepParams parms)
    {
        ...
        {
            Log.Warning("GenStep_PHPrison: Could not find valid room location");
            return;
        }
    ...
    private void SpawnHunterGuards(Map map, IntVec3 roomTopLeft)
    {
        ...
        FactionDef hunterFactionDef = PawnHuntersDefOf.PH_PawnHunters;
        ...
        for (int i = 0; i < guardCount; i++)
        {
            ...
            PawnKindDef guardKind = Rand.Bool ? PawnHuntersDefOf.PH_PawnHuntersFighter_Ranged : PawnHuntersDefOf.PH_PawnHuntersFighter_Melee;
    ...
    private void PopulateBedroom(Map map, IntVec3 roomTopLeft)
    {
        ...
        if (GenSpawn.Spawn(MakeThingWithDefaultStuff(ThingDefOf.Shelf), shelfPos, map) is Building shelf)
        {
            ...
            // 5% chance: a single bloodprime on the shelf
            if (Rand.Chance(0.05f))
            {
                Thing bp = ThingMaker.MakeThing(PawnHuntersDefOf.PH_BP);
                GenPlace.TryPlaceThing(bp, shelfPos, map, ThingPlaceMode.Near);
            }
        }
    }
